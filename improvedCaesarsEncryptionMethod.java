...
public class improvedCaesarsEncryptionMethod {
					...
					if (input.indexOf("-bf")!=-1) {
						...
						if (input.indexOf("sce")!=-1) {
							...
							if (input.indexOf("-p")!=-1) {
								...
								System.out.println("Please supply the appropriate key that was used to encrypt the message."+System.lineSeparator()+"Ensure it is valid.");
						...
						if (input.indexOf("sce")!=-1) {
							...
							if (input.indexOf("-p")!=-1) {
								...
								System.out.println("Please supply the appropriate key to be used during encryption."+System.lineSeparator()+"Ensure it is valid.");
								String supplied = System.console().readLine();
								String type = "";
								for (int i = 0; i < supplied.length(); i++) {
									if (Character.isDigit(supplied.charAt(i))) {
	...
	public void helpDocs() {
		...
		System.out.println("");
		System.out.println("Using the special key correctly:");
		System.out.println("");
		System.out.println("As a bipolar multiplier or key (ECE & SCE perspectively) : Simply input"+System.lineSeparator()+"the number you wish to be used during the encryption/decryption process.");
		System.out.println("");
		System.out.println("As a function : Make sure that the function follows this program's input rules."+System.lineSeparator()+"The rules are as follows;"+System.lineSeparator()+System.lineSeparator()+"   i) Mathematical operations (\"+\", \"-\" and the like) need to be explicitly"+System.lineSeparator()+"   prioritized with parenthesis (\"(\" and \")\"). The innermost will be the one"+System.lineSeparator()+"   that will be executed first."+System.lineSeparator()+System.lineSeparator()+"   ii) Mathematical operators accepted are the following signs as their common"+System.lineSeparator()+"   function except for @, which denotes a square root: \"+\",\"-\",\"*\",\"^\",\"%\",\"@\""+System.lineSeparator()+"   and \"/\"."+System.lineSeparator()+System.lineSeparator()+"   iii) The variable should only be written as \"x\". Any other symbol used won't"+System.lineSeparator()+"   be recognised."+System.lineSeparator()+System.lineSeparator()+"   iv) An inner and outer left parenthesis should be connected by a maximum of 1"+System.lineSeparator()+"   mathematical operation. f.e. (3+2-(1+x)) is a wrong representation but"+System.lineSeparator()+"   (3-(1+x)+2) is a correct one.");
		System.out.println("");
